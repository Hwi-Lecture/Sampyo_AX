...
2025년 국가별(AU/ID/US) 석탄(HS 2701) 수입 통계를 계산하는 모듈.
...
이 모듈의 fetch_trade_stats / aggregate_monthly / build_summary 는
main.py(FastAPI 백엔드)가 그대로 import 해서 사용합니다.

단독 실행 시(python load_data.py)에는 기존처럼 dashboard_data.js 파일을
생성합니다 (서버 없이 dashboard.html을 더블클릭으로 열어보고 싶을 때 용도).

사용법:
    1. .env 파일에 API_KEY=발급받은_인증키(Decoding) 입력
    2. pip install -r requirements.txt
    3-a. 서버로 실행: uvicorn main:app --reload
    3-b. 정적 파일만 생성: python load_data.py
"""
